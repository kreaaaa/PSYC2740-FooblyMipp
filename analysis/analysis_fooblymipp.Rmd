...
education_distribution= demographics%>%
  ...
  count(education)
```
#Our mean age is 33.63492 years old with a standard deviation of 14.70325. (M = 33.63, SD = 14.70325)
...
#Education distribution: 5 years (n = 1), 8 years (n = 1), 10 years (n = 3), 11 years (n = 2), 12 years (n = 35), 13 years, (n = 10), 14 years (n = 15), 15 years (n = 13), 16 years (n = 19), 17 years (n = 10), 18 years (n = 5), 19 years (n = 4), 20 years (n = 5), 21 years (n = 1), 24 years (n = 1), 27 years (n = 1). 
...
```
# Average accuracy for target trials (not practice trials) with no exclusions (M = 0.8858333, SD = 0.3181462)
...
```{r}
priming_model = lm(data = priming_data, rt ~ condition + relatedness +condition:relatedness)

car :: Anova(priming_model)
```
#An ANOVA was conducted. No main effect of Condition was reported (F(1, 399)= 0.095, p = 0.392.
#There is a main effect of Relatedness (F(1, 399)= 13.807, p < 0.001).
#There is also a significant interaction between gender and education (F(1, 399)= 6.619, p = 0.010)


#Using this for pairwise comparisons
```{r}
emmeans :: emmeans (priming_model,
                    pairwise~ condition | relatedness,
                    adjust = "tukey")
```

#Looking for something going on in the contrast area.
#Tukey adjust follow-up t-tests indicate that rhyming vs non-rhyming interaction level does not deviate significantly for the unrelated condition, but rhyming pairs have lower mean reaction times than non-rhyming pairs for related pairs.  
...
```{r}
library(performance)
check_model(priming_model)
```
#Ours is not perfect, but it is okay. No major violations in assumptions were depicted here. (It seems slightly off for normality of residuals, but we do not think that it is significant enough to violate the assumption)
...
```
#For relatedness χ² = 0.843.86, df = 3, p > 0.001
#For condition χ² = 3.21, df = 1, p = 0.073
#We have no major violations of assumptions with this model.
#A Chi-squared test of independence was conducted to examine the relationship between Condition and Relatedness. The results showed no significant association between the variables, χ² = 5.76, df = 3, p = 0.12.
